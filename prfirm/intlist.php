	...
	<html>
		<head>
			...
			<script>
				...
				function deleterecord(recname, recid) 
				{
					...
					if (confirm(sText))
					{
						document.forms[0].action = "deleteintfirm.php";
		...
		<tr>
			...
			<?php 
				...
				$query = 'SELECT id, name, city, country, phone, inactive, userid, validated FROM prfirm WHERE inactive = 0 and country = "'.$_POST['country'].'" ORDER BY name';
				

				$query2 = 'SELECT count(*) as mcount FROM prfirm WHERE country = "'.$_POST['country'].'"';
			...
			<?php
					while( $row=mysql_fetch_array($result) )
					{
						...
						echo '</td>';
						echo '<td><a href="editfirm.php?sessionid='.$session->SessionID.'&id='.$row['id'].'" target="main">'.$row['name'].'</a></td>';
						echo '<td>'.$row['city'].'</td>';
