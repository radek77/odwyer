<?php 
	...
	$query = 'SELECT * FROM corp WHERE dirtype = 2 and inactive = 0 and company like "' . $_POST["fieldvalue"] . '%" ORDER BY company';
	...
	while( $row=mysql_fetch_array($result) )
	{
		...
		*/
		...
		{			
			...
			$pos1 = strpos(strtolower($row['miscinfo1']), "fax");
			$pos2 = strpos(strtolower($row['miscinfo1']), $phone);
		...
		{			
			...
			$pos = strpos(strtolower($row['miscinfo1']), $url);
...
","",$miscinfo);
		...
		if ($row['presceo']!='')
		{
			$text = $text . $row['presceo'];
			
			if($row['prtitle']!='')
			{
				$text = $text . ", " . $row['ceotitle'];
			}
